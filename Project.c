...
int main() {
    ...
    do {
        ...
        if (scanf("%d", &choice) != 1) {
            ...
            flush_input(); // Clear invalid input
            continue;
        }
...
void student_portal() {
    ...
    do {
        ...
        if (scanf("%d", &choice) != 1) {
            printf("Invalid input. Please enter a number between 1 and 4.\n");
            flush_input(); // Clear invalid input from buffer
            continue;
        }
...
void solve_complaint() {
    ...
    // First, check for high-priority complaints and unsolved
    for (int i = 0; i < complaintCount; i++) {
        if (complaints[i].priority == 1 && complaints[i].status == 0) { 
    ...
    // If no high priority complaints were found, solve the first low priority complaint and unsolved
    if (!found) {
        for (int i = 0; i < complaintCount; i++) {
            if (complaints[i].priority == 0 && complaints[i].status == 0) {  
                complaints[i].status = 1;  // Mark the complaint as solved
                found = 1;
                return;  
            }
